...
while(1):
    ...
    if ballSprite.x <= 0 and ballDir == 0: 
        ballDir = 1
    elif ballSprite.x <= 0 and ballDir == 3: 
        ballDir = 2
    elif (ballSprite.x + 4) >= 72 and ballDir == 1: 
        ballDir = 0
    elif (ballSprite.x + 4) >= 72 and ballDir == 2: 
        ballDir = 3
    elif ballSprite.y <= 0 and ballDir == 2: 
        ballDir = 1
    ...
    # CHECK IF WON GAME
    if gameScore == 30:
        thumby.display.drawText("You won!", 18, 5, 1) # text, x, y, color
        thumby.display.drawText("Play again? A/B", 0, 32, 1)
